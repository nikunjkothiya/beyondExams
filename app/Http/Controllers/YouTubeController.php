...
class YouTubeController extends Controller
{
  ...
  public function get_video_all_details(Request $request)
  {
    ...
    try {
      $final_response = [];
      $request = implode(',', $request->video_urls);
      //foreach ($request->video_urls as $video_url) {
      //  $request = $video_url;
      //  $base_url = env('YOUTUBE_DATA_BASE_URL');
      $base_url = env('YOUTUBE_MULTIPLE_VIDEO_IDS_DETAILS_URL');
      $key = env('YOUTUBE_DATA_API');
      $maxResults = 50;

      $curl = curl_init();

      $url =  $base_url . '?part=snippet&id=' . $request . '&fields=items(id,snippet.title,snippet.description)&key=' . $key;
